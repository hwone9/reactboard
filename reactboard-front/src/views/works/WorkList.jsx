...
const WorkList = () => {
    ...
    const getWorkList = () => {//리스트 조회
        let resp = util.callApi("get",`/work`, null);
        if (resp.success) {
            resp = resp.resData;
    ...
    const insertWorkItem = ()=>{//리스트 추가 함수
        if(workItem){
            let param = {"work" : workItem};
            let resp = util.callApi("post",`/work`, param);
            if (resp.success) {
                resp = resp.resData;
                setWorkItem('');
                getWorkList();
    ...
    const onClickDoneBtn = (idx)=>{//완료여부
        let param = {
            "idx" : idx
          }
        let resp = util.callApi("patch",`/workdone`, param);
        if (resp.success) {
            resp = resp.resData;
            getWorkList();
        }
    ...
    const updateWork = (param)=>{//입력된 할일 focus out 발생 시 데이터 업데이트
        console.log(param);
        return;
        ...
        //     };
        //     let resp = util.callApi("patch",`/work`, param);
        //     if (resp.success) {
        //         resp = resp.resData;
        //         getWorkList();
        //     }   
    ...
    const onClickDeleteBtn = (idx)=>{//등록된 할 일 삭제
        let resp = util.callApi("delete",`/work/${idx}`);
        if (resp.success) {
            resp = resp.resData;
            setWorkItem('');
            getWorkList();
